...
import { readStage2, writeStage2 } from '@/lib/stage2Store';
import { isAuthedViaCookies } from '@/lib/auth';

export async function GET() {
  const ids = await readStage2();
...
export async function DELETE(req: NextRequest) {
  ...
  const set = new Set(await readStage2());
